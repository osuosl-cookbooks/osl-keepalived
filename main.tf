...
resource "openstack_networking_port_v2" "node1_port" {
    network_id            = "${openstack_networking_network_v2.openstack_network.id}"
    admin_state_up        = "true"
...
resource "openstack_networking_port_v2" "node2_port" {
    network_id            = "${openstack_networking_network_v2.openstack_network.id}"
    admin_state_up        = "true"
    port_security_enabled = "false"
    fixed_ip {
...
resource "openstack_compute_instance_v2" "node1" {
    ...
    network {
        port = "${openstack_networking_port_v2.node1_port.id}"
    }
    provisioner "chef" {
...
resource "openstack_compute_instance_v2" "node2" {
    ...
    network {
        port = "${openstack_networking_port_v2.node2_port.id}"
    }
    provisioner "chef" {
        run_list        = ["recipe[keepalived_test::provisioning]"]
        node_name       = "node2"
